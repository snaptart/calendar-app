...
const EventBus = (() => {
	// Wait for IceTimeApp to be ready, or create fallback
	if (window.IceTimeApp && window.IceTimeApp.EventBus) {
		return window.IceTimeApp.EventBus;
	}

	// Fallback EventBus if core.js hasn't loaded yet
	const events = {};
	return {
		on(event, callback) {
			if (!events[event])
